...
const packageRoutes = require("./routes/packagesroute"); // Fixed path
...
// Get all users (Admin only) - Line 30 where the error occurs
...
// Update your PUT route in paste.txt
app.put("/api/destinations/:id", [requireAuth, requireAdmin], async (req, res) => {
  const { id } = req.params;
  const { name, image, description, dtype } = req.body; // Include dtype
  ...
  try {
    ...
    destination.dtype = dtype || destination.dtype; // Update dtype if provided
...
// Add this to your server code (paste.txt)
